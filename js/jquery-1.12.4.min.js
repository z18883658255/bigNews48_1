...
//每一个ajax都要设置令牌,不然人家以为你没有登录,就不给你数据
//每一个ajax都要设置令牌,所以我们可以全局设置

//使用ajaxSetup()方法来配置全局的信息
//吧$.ajaxSetup写在jquery文件中
//是因为不管那个页面使用jq发送ajax请求,都要先导入jq这个文件那下面这个方法就会执行了
$.ajaxSetup({
  //发送ajax请求前会运行这个函数,所以在这个函数重设置带过去的token令牌
  beforeSend(xhr){
